...
description: A projection view is a compiled artifact of project truth. It should be generated on demand for usage contexts rather than maintained as an external knowledge base; projections are read-only, and corrections land on project truth before the projection is regenerated.
dependsOn:
...
Projection views are read-only. When content in a projection needs correction, the correction should land on project truth, and the projection should then be regenerated.

> This article builds on the definition of projection views as derived results in [What Is Project Truth](/en/argument/what-is-project-truth/), and discusses how projection views should be produced and maintained.

## Background

Project truth serves many usage contexts. Taking on a new task calls for a digest of the relevant background, new members need an onboarding tour, audits need a cross-domain view of rules, and related projects may need background knowledge about one of this project's domains. These contexts require different ways of extracting and presenting content.
...
The central question is therefore: should presentations aimed at usage contexts exist as parallel materials that require continuous synchronization, or as derived results that can be regenerated from project truth at any time?

## Rationale

**Project truth is the single source; projection views are compiled artifacts.**

Project truth already carries everything the current project actually follows. A projection view adds no truth; it only changes the presentation, extracting and reorganizing content that already exists in project truth for a usage context. This relationship mirrors that of source code and build artifacts: build artifacts can be cached, distributed, and discarded; when change is needed, the source is modified and the artifact rebuilt.

Treating projection views as compiled artifacts means every presentation consumed by people and AI can be traced to some version of project truth. Whether a presentation can be trusted comes down to whether the source is trustworthy and whether the version is current — both questions have definite answers.
...
**Read-only projections keep project truth singular.**

While consuming projections, people and AI will find content that needs correction. If the correction lands on the projection, that projection leaves the regenerable state and starts evolving separately from project truth; later consumers face two diverging materials and cannot tell which one to follow.

If the correction lands on project truth, every subsequent projection receives it automatically. This discipline is the same one as not hand-editing build artifacts: problems found in the artifact are fixed at the source. It guarantees that project truth remains the only object that needs maintenance.
...
Organizing results fall into two kinds, each with its own destination. The way of organizing — the structures, templates, and tradeoffs for arranging material for a class of contexts — should settle into compiler configuration and be reused across versions. New conclusions discovered during organizing — a clearer statement of a rule, a constraint not previously made explicit — should flow back into the grounding part of project truth if the project still follows them.
...
Faster now, expensive later. The moment a projection is edited, it enters a state that requires manual maintenance, and its divergence from project truth can no longer be decided by version. Allowing exceptions for individual edits forfeits the entire chain of decidability.

The need for fast correction should be met by lowering the cost of going back to the source: making it cheap to change project truth and regenerate the projection. Routing corrections through project truth also routes them through the normal review boundary, rather than scattering them across projection copies.
...
The same tool can slide between the two modes of operation. The criteria this article provides: can the content be regenerated from project truth, and do corrections return to project truth?

## Conclusion

Projection views should be compiled on demand. Project truth is the single source, and projection views are its compiled artifacts for usage contexts. On-demand generation removes the synchronization burden of parallel knowledge bases, version boundaries make cache staleness decidable, and domain organization allows regeneration to happen locally.

Projection views are read-only. Problems found in a projection are corrected in project truth, and the projection is then regenerated. This discipline keeps project truth singular: the presentation people and AI receive in any usage context can be traced to the current version of project truth.
