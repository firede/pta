---
title: Project Truth Must Live in the Codebase
description: In software projects where coding agents participate in long-term iteration, project truth must live in the codebase and share a version boundary with the implementation.
dependsOn:
  - argument/what-is-project-truth
sourceHash: d8347e8f2ce954bbe3599dcaec3394ff703a71d76aa1c6c3c3e223669710968e
---

In software projects where coding agents participate in long-term iteration, project truth must live in the codebase and share a version boundary with the implementation.

This article follows the definition in [What Is Project Truth](/en/argument/what-is-project-truth/): project truth is the content the current project actually relies on. Part of it exists in the code implementation, and another part is supplemented by natural language, glossaries, decision records, and necessary historical memory.
...
The value of requirement materials lies in constraining and guiding implementation. Long-term iteration requires the content the current project actually relies on to be kept in the same version system as the implementation, so that it is not left in external documents, human memory, meeting notes, or expired issues.
...
External knowledge bases are still valuable. They are suitable for reading, discussion, presentation, and material collection. Their weakness lies in the version boundary: they do not share commit history, review process, rollback boundary, and change context with code. For long-term iteration, if the content the current project actually relies on does not evolve with the implementation, it will gradually become unreliable out-of-band information.
...
Team meetings, user feedback, product discussions, multi-turn clarification with coding agents, and pull requests triggered automatically by feedback can all produce project truth. These processes themselves do not need to enter the codebase in their original form. Only content that has been clarified, compressed, and confirmed, and has become content the current project actually relies on, needs to be captured as the current version's business state, domain constraints, real-world conditions, commercial constraints, product semantics, stable terminology, decision outcomes, and key reasons for change.
...
Project truth must live in the codebase because the content the current project actually relies on needs to share the same version boundary as the implementation. This lets people, agents, and tools work from the same current basis and reduces the need to carry context back and forth between external materials and code changes.
