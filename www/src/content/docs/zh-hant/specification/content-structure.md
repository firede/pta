...
條目是正文的最小內容單元。人的裁決與領域間引用以條目為落點，核查信號的落點與處置由治理規範定義；工具以條目內容的雜湊識別條目，內容變化即身分變化，掛靠其上的推導結果與裁決隨之失效。快取的儲存形態與失效規則由治理規範定義。
...
術語條目同樣以行首的 `- ` 開啟，至行尾結束，**不得** 跨越多行；條目內與清單外的區塊級結構約束，與判斷條目相同。
