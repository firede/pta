...
test('init 创建 pta.toml 声明工作语言，已存在或标签不合形制时拒绝', async (context) => {
  ...
  assert.equal(await runCli(['init', 'zh-Hans'], created.io, root), 0);
