...
test('assembleContext 限定文件的外置领域优先于目录覆盖，路径指向的目录领域为上级', () => {
  const frontmatter = splitFrontmatter('- 判断\n').frontmatter;
  const external: Domain = {
    kind: 'external',
    declarationPath: '.pta/slice/TRUTH.md',
    containerPath: '.pta/slice',
    identifier: '.pta/slice',
    claimedPath: 'src',
    filesPresent: true,
    files: ['special.ts'],
    dependsOn: [],
  ...
  const result = discovery([domain(''), domain('src'), external]);
  const byId = new Map(result.domains.map((item) => [item.identifier, item]));
  const contents = [
    content(byId.get('') as Domain, { 'TRUTH.md': '- 根判断\n' }),
    content(byId.get('src') as Domain, { 'TRUTH.md': '- 源判断\n' }),
    content(byId.get('.pta/slice') as Domain, { 'TRUTH.md': '- 切片判断\n' }),
  ];

  const assembly = assembleContext(result, contents, ['src/special.ts', 'src/other.ts']);

  assert.deepEqual(
    assembly.resolutions.map((item) => [item.path, item.domainIdentifier]),
    [
      ['src/special.ts', '.pta/slice'],
      ['src/other.ts', 'src'],
    ],
  );
  assert.deepEqual(
